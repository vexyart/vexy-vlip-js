...
- **Web Component** (`src/element.js`): `<vexy-vlip>` auto-registers on import; maps kebab-case attributes to options; `src`, `track`, `vtt`, and `poster` changes rebuild the player; `mode` and `easing` changes delegate live; Shadow DOM scoped styles; delegated methods and getters on the host element.
...
- **18 `node --test` unit tests** covering the VTT parser (plain text, JSON, fallback, markdown, cue settings) and segment/stop-point logic (pure functions, no DOM).
- **6 Playwright-library e2e checks** (`tests/e2e/run-e2e.mjs`): ready/segment count, stepped pause-at-first-cue with card shown, advancing twice, continuous card-on-play, control-bar dot count, and `prev()` navigation; the runner uses the Playwright library API directly (not `@playwright/test`) to avoid ESM loader issues under Node 26.
- **GitHub Pages demo site** (`docs/`): landing page with live demo, plus `demo-module.html`, `demo-component.html`, and `demo-library.html`; deployed automatically by GitHub Actions on push to `main`.
